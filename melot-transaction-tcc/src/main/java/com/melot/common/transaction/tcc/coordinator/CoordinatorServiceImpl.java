...
import com.melot.common.transaction.tcc.TccSpringBeanFactory;
import com.melot.common.transaction.tcc.config.TccConfig;
import com.melot.common.transaction.tcc.repository.TransactionRepository;
                    ...
                    if (tccTransactions != null && tccTransactions.size() > 0) {
                        ...
                        for (TccTransaction tccTransaction : tccTransactions) {
                            ...
                            if (tccTransaction.getRole() == TccRole.PROVIDER.getRole() &&
                                    tccTransaction.getStatus() == TccStatus.PRE_TRY.getStatus()) {
                                continue;
                            }
                            ...
                            try {
                                ...
                                if (rows > 0) {
                                    //如果是以下3种状态
                                    if ((tccTransaction.getStatus() == TccStatus.TRYING.getStatus()
                                            || tccTransaction.getStatus() == TccStatus.PRE_TRY.getStatus()
